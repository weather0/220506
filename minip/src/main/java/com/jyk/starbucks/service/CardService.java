...
public class CardService {
	...
	public List<CardFace> cardDisplay() { // CARDSFACE테이블의 VO로서 CardFace타입으로 ArrayList에 담음
		...
		try {
			...
			while (rs.next()) { // {커서를 다음행으로 이동(next)하면서 쿼리탐색(rs). CardFace타입의 vo의 요소를 set해서 완성(쿼리결과를 끌어오려면
								// rs기능으로 get). 그 vo를 display배열 한 줄에 add} → 이걸 계속 반복(while)
				vo = new CardFace();
				vo.setCardorder(i++); // 초기값 1을 먼저 담고 1을 증가(++) → 다음루프반복
				vo.setCard_name(rs.getString("card_name"));
				display.add(vo);
				// display = {CardFace,CardFace,...,CardFace}
				// = {(order,name,avl),(order,name,avl),...,(order,name,avl)}
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
